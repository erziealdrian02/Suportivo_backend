        ...
        redirect('user/index2');
			...
			public function edit2($id=0)
	      {
	          if($_POST){
	            ...
	            $data =array();
	            $xxdata = $this->db->get_where('berita', array('id_berita' => $id))->row();
	            $this->db->delete('berita', array('id_berita' => $id));
	            for($i=0; $i<$count; $i++) {
	            $data = array(
	                 'judul_berita' => $_POST['judul'][$i],
	                 'isi_rangkum' => $_POST['value'][$i],
	                 'isi_full' => $_POST['urut'][$i],
	                 );

	             $this->db->insert('berita', $data);
	                }
	                redirect('user/daftarberita');
	          }
	      }
    ...
    public function hapus($id=0)
      {
		...
		public function hapus_berita($id_berita){
	  		...
	  		$this->GambarModel->hapus_databarang($where,'berita');
	  		redirect('user/managenews');
	 	}
